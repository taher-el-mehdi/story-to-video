...
# === CONFIG ===
STORIES_DIR = os.path.join("stories", "02 - Small fry", "story_cleaned")
OUTPUT_DIR = os.path.join("stories", "02 - Small fry", "story_audios")
...
async def generate_audio(file_path: str) -> None:
    try:
        ...
        async with openai.audio.speech.with_streaming_response.create(
            ...
            output_path = file_path.replace("story_cleaned", "story_audios").replace(".txt", ".wav")
...
async def main() -> None:
    cwd = os.getcwd()
    full_pattern = os.path.join(cwd, STORIES_DIR, "*.txt")
